    ...
    const updateUser = async (values) => {
        ...
        try {
            ...
            const userUpdate = await fetch(
                (process.env.REACT_APP_SERVER ? process.env.REACT_APP_SERVER : `http://localhost:5000/`) + `cuidadores/${user.id}`,
                {
                    method: "PUT",
                    headers: {
					...
					<Form>
						<div className='fixed inset-0 bg-gray-800 bg-opacity-40 z-50 flex justify-center items-center'>
							<div className='flex flex-col relative w-5/6 max-h-screen overflow-y-scroll bg-gray-100 p-7 rounded-md'>
								...
								<p className='font-bold my-2 text-black text-center'>Edite sus datos</p>
